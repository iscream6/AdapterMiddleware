...
namespace NetworkCore
{
    ...
    public class TcpServer : IDisposable
    {
        ...
        public virtual bool SendtoSesstion(Guid id, string text)
        {
            return SendtoSesstion(id, Encoding.UTF8.GetBytes(text));
        }

        public virtual bool SendtoSesstion(Guid id, byte[] buffer)
        {
            return SendtoSesstion(id, buffer, 0, buffer.Length);
        }

        public virtual bool SendtoSesstion(Guid id, byte[] buffer, long offset, long size)
        {
            if (!IsStarted)
                return false;
            ...
            TcpSession session = Sessions.TryGetValue(id, out TcpSession result) ? result : null;
            session?.SendAsync(buffer, offset, size);
